...
int8_t i2cm_ReadBuffAndStop(I2C_TypeDef* I2Cx, uint8_t *pbuf, uint16_t len, uint16_t TimeOut)
{
  ...
  if (len == 1)
  {
    ...
    // EV6_1 В реализации поллингом приходится запрещать прервания
    __disable_irq();
    (void) I2Cx->SR2;                           // Очищаем флаг ADDR                           
    I2C_GenerateSTOP(I2Cx,ENABLE);              // Выдаём условие STOP
    __enable_irq();

    // EV7 Принимаем последний байт
    TOcntr = TimeOut;
  ...
  {
    // Следующий байт не подтверждаем
    I2C_NACKPositionConfig(I2Cx, I2C_NACKPosition_Next);

    // EV6_1 В реализации поллингом приходится запрещать прервания
    __disable_irq();
    (void) I2Cx->SR2;                           // Очищаем флаг ADDR
    I2C_AcknowledgeConfig(I2Cx, DISABLE);       // Запрещаем выдачу ACK      
    __enable_irq();

    // EV7_3 Ждём флаг BTF, выдаём STOP
    TOcntr = TimeOut;
    while ((!I2C_GetFlagStatus(I2Cx, I2C_FLAG_BTF)) && TOcntr) {TOcntr--;}
  ...
  else 
  {
    ...
    while (len-- != 3)
    {
      // EV7 Ждём флаг BTF, читаем очередной байт
      TOcntr = TimeOut;
      while ((!I2C_GetFlagStatus(I2Cx, I2C_FLAG_BTF)) && TOcntr) {TOcntr--;}
      *pbuf++ = I2C_ReceiveData(I2Cx);
    }
    ...
    *pbuf++ = I2C_ReceiveData(I2Cx);             // Читаем N-1 байт
